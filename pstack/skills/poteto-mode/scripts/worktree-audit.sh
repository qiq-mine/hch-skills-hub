...
#
# Supports Antigravity, DSH, Claude Code, and Cursor environments.
...
# Discover transcript directory across Antigravity, DSH, and Cursor
...
git worktree list --porcelain | awk '/^worktree /{print $2}' | while read -r wt; do
	...
	# Most recent session whose transcript operated in this worktree.
	last="-"; last_ts=0
	if [ -n "$transcripts" ] && [ -d "$transcripts" ]; then
		f=$(grep -rl -e "${wt}/" -e "${wt}\"" "$transcripts" 2>/dev/null | head -1 || true)
		if [ -n "$f" ]; then
			# Support both Linux (stat -c) and BSD/macOS (stat -f)
			last_ts=$(stat -c '%Y' "$f" 2>/dev/null || stat -f '%m' "$f" 2>/dev/null || echo 0)
			if [ "$last_ts" -gt 0 ] 2>/dev/null; then
				last=$(date -d "@$last_ts" '+%Y-%m-%d' 2>/dev/null || date -r "$last_ts" '+%Y-%m-%d' 2>/dev/null || echo "-")
			fi
		fi
	fi
