...
- **Coordinator (this chat).** Frames, authors briefs, drains the inbox, owns the human report, makes judgment calls. It never authors or edits code directly. Conflicted merges, restacks, and code changes are always delegated to subagent tasks. Mechanically landing a verified unit (fast-forward or clean cherry-pick of a worker's commit, then push) is bookkeeping the coordinator may do itself. Subagents are spawned, messaged, and managed through agent tools (`invoke_subagent`, `manage_task`, `send_message`). State reads and writes go through `scripts/orch/orch.ts` at drain points.
- **Sub-coordinator.** Durable, one per track, used only when the program exceeds what one coordinator can manage. Owns its track's units, authors worker briefs, spawns its workers and verifiers. Rolls up aggregates at wave boundaries. Never forwards raw child reports.
- **Worker / verifier.** Spawned as subagents with isolated branches (`Workspace: "branch"` in Antigravity). One writer per worktree or branch (principle-separate-before-serializing-shared-state). Run a unit's verifier on a different model tier from its worker.
...
- Never resume an agent to check on it. A resume restarts an idle agent. Probe read-only: the ledger, `units.tsv`, `gh`, pushed branches, or task status. Transcript mtime is not liveness.
...
- After a session restart: re-read the standing orders and `units.tsv`, recompute the frontier, reattach work by PR and branch, respawn coordinators from stored briefs plus current state, drain, resume.
