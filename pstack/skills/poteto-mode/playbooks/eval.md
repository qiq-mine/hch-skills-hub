...
6. **Verify the chain from transcripts, not self-report.** Read each candidate's transcript (in Antigravity: under the active workspace's logs at `<appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl`; in DSH: from the runtime session directory). Look at which files each candidate actually opened. Grade chain-following from the files it really read plus the shape of the code, never from the candidate's own claims.
