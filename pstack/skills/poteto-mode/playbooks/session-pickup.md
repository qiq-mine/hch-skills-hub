...
1. Locate the prior trail: in Antigravity, read transcripts from `<appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` (or linked conversation transcripts); in DSH, read the session log directory; or inspect git commit history and pushed branches. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long transcript in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).
