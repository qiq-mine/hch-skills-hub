...
**Control skill.** Pick it by surface. Browser, Electron, and web UIs use browser/Playwright tools or project verification skills (from `create-verification-skill`). CLIs and TUIs use terminal/PTY test scripts. Native mobile uses simulator-driving tools. A PR that touches two surfaces gets lanes on both. A surface with no control skill is a risk in Appendix C, and its live block still names how each lane drives it.
...
Each live lane runs in its own isolated environment at the PR head. Drive through browser/Playwright tools, test harnesses, or CLI scripts.
