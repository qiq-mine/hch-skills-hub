...
In Antigravity, conversation transcripts live at `<appDataDir>/brain/<conversation-id>/.system_generated/logs/transcript.jsonl` (and `transcript_full.jsonl`). In DSH, transcripts live in the session log directory.
...
2. Lock the scope before searching. Pin the window ("recent" is a real range, default the last 7 days), the topic if named, and the workspace. State the scope back. Never quietly turn "all" into "recent N".
3. Fan out across chat history. In Antigravity, spawn parallel subagents via `invoke_subagent` with `TypeName: "research"` and `Model: "flash"`; in DSH, spawn parallel worker subagents. Each takes a slice of the conversation corpus. Tell every subagent to order candidates by real modification time (`ls -t`), grep the topic first, read only matching sessions and relevant regions, and skip trivial noise. Each returns the same schema: topic, user goal, decisions, open threads, corrections, and artifacts. The raw transcripts stay in the subagents. The main thread gets only their findings.
